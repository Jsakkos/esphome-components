...
static const char *const TAG = "scd4xx";

static const uint16_t SCD4XX_CMD_GET_SERIAL_NUMBER = 0x3682;
static const uint16_t SCD4XX_CMD_TEMPERATURE_OFFSET = 0x241d;
static const uint16_t SCD4XX_CMD_ALTITUDE_COMPENSATION = 0x2427;
static const uint16_t SCD4XX_CMD_AMBIENT_PRESSURE_COMPENSATION = 0xe000;
static const uint16_t SCD4XX_CMD_AUTOMATIC_SELF_CALIBRATION = 0x2416;
static const uint16_t SCD4XX_CMD_START_CONTINUOUS_MEASUREMENTS = 0x21b1;
static const uint16_t SCD4XX_CMD_GET_DATA_READY_STATUS = 0xe4b8;
static const uint16_t SCD4XX_CMD_READ_MEASUREMENT = 0xec05;
static const uint16_t SCD4XX_CMD_PERFORM_FORCED_CALIBRATION = 0x362f;
static const uint16_t SCD4XX_CMD_STOP_MEASUREMENTS = 0x3f86;

static const float SCD4XX_TEMPERATURE_OFFSET_MULTIPLIER = (1 << 16) / 175.0f;

void SCD4XXComponent::setup() {
  ESP_LOGCONFIG(TAG, "Setting up scd4xx...");

  // the sensor needs 1000 ms to enter the idle state
  this->set_timeout(1000, [this]() {
    // Check if measurement is ready before reading the value
    if (!this->write_command_(SCD4XX_CMD_GET_DATA_READY_STATUS)) {
    ...
    if (raw_read_status[0]) {
      ...
      if (!this->write_command_(SCD4XX_CMD_STOP_MEASUREMENTS)) {
        ESP_LOGE(TAG, "Failed to stop measurements");
        this->mark_failed();
        return;
      }
      // According to the Scd4xx datasheet the sensor will only respond to other commands after waiting 500 ms after
      // issuing the stop_periodic_measurement command
      stop_measurement_delay = 500;
    }
    this->set_timeout(stop_measurement_delay, [this]() {
      if (!this->write_command_(SCD4XX_CMD_GET_SERIAL_NUMBER)) {
      ...
      if (!this->write_command_(SCD4XX_CMD_TEMPERATURE_OFFSET,
                                (uint16_t)(temperature_offset_ * SCD4XX_TEMPERATURE_OFFSET_MULTIPLIER))) {
      ...
      if (ambient_pressure_compensation_) {
        ...
        if (!this->write_command_(SCD4XX_CMD_ALTITUDE_COMPENSATION, altitude_compensation_)) {
      ...
      if (!this->write_command_(SCD4XX_CMD_AUTOMATIC_SELF_CALIBRATION, enable_asc_ ? 1 : 0)) {
      ...
      if (!this->write_command_(SCD4XX_CMD_START_CONTINUOUS_MEASUREMENTS)) {
...
void SCD4XXComponent::dump_config() {
  ESP_LOGCONFIG(TAG, "scd4xx:");
  LOG_I2C_DEVICE(this);
  if (this->is_failed()) {
...
void SCD4XXComponent::update() {
  ...
  if (!this->write_command_(SCD4XX_CMD_READ_MEASUREMENT)) {
...
void SCD4XXComponent::set_ambient_pressure_compensation(float pressure_in_bar) {
...
bool SCD4XXComponent::update_ambient_pressure_compensation_(uint16_t pressure_in_hpa) {
  if (this->write_command_(SCD4XX_CMD_AMBIENT_PRESSURE_COMPENSATION, pressure_in_hpa)) {
...
uint8_t SCD4XXComponent::sht_crc_(uint8_t data1, uint8_t data2) {
  uint8_t bit;
  uint8_t crc = 0xFF;
...
bool SCD4XXComponent::read_data_(uint16_t *data, uint8_t len) {
...
bool SCD4XXComponent::write_command_(uint16_t command) {
...
bool SCD4XXComponent::write_command_(uint16_t command, uint16_t data) {
  uint8_t raw[5];
  raw[0] = command >> 8;
  raw[1] = command & 0xFF;
  raw[2] = data >> 8;
